  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (tab === "register") {
        ...
        setLoading(false);
        if (authError) {
          setError(authError.message.includes("already") ? "Diese E-Mail ist bereits registriert." : authError.message);
          return;
        }

        if (data.session) {
          onSuccess?.({ id: data.user!.id, email: data.user!.email!, name: form.name, avatar: `https://i.pravatar.cc/150?u=${form.email}` });
          onClose();
          router.refresh();
        } else {
        ...
        onSuccess?.({
          ...
          avatar: meta.avatar ?? `https://i.pravatar.cc/150?u=${data.user.email}`,
        });
        onClose();
        router.refresh();
      }
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-sm rounded-2xl border border-white/10 bg-[#0B1420] p-6 shadow-2xl">
        <button onClick={onClose} className="absolute right-4 top-4 p-1 text-white/40 transition-colors hover:text-white" aria-label="Schliessen">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6 6 18M6 6l12 12" /></svg>
        </button>

        <div className="mb-6">
          {reason && <p className="mb-1 text-sm text-white/50">Bitte einloggen {reason}</p>}
          <h2 className="text-xl font-semibold text-white">{tab === "login" ? "Anmelden" : "Konto erstellen"}</h2>
        </div>

        <div className="mb-5 flex rounded-xl bg-white/5 p-1">
          {(["login", "register"] as const).map((t) => (
            <button key={t} onClick={() => { setTab(t); setError(""); setSuccess(""); }}
              className={`flex-1 rounded-lg py-1.5 text-sm font-medium transition-all ${tab === t ? "bg-[rgba(45,200,190,0.15)] text-[#2DC8BE]" : "text-white/40 hover:text-white/70"}`}>
        ...
        <form onSubmit={submit} className="space-y-3">
          {tab === "register" && (
            <>
              <div>
                ...
                <input required value={form.name} onChange={set("name")} placeholder="Dein Name"
              ...
              <div className="flex gap-2">
                <div className="flex-1">
                  ...
                  <input value={form.location} onChange={set("location")} placeholder="Zürich"
                    className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder-white/30 outline-none focus:border-[rgba(45,200,190,0.5)]" />
                </div>
                <div className="flex-1">
          ...
          <div>
            ...
            <input required type="email" value={form.email} onChange={set("email")} placeholder="deine@email.ch"
              className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder-white/30 outline-none focus:border-[rgba(45,200,190,0.5)]" />
          </div>
          ...
          {success && <p className="rounded-lg bg-green-500/10 px-3 py-2 text-xs text-green-400">{success}</p>}

          <button type="submit" disabled={loading}
            className="mt-1 w-full rounded-xl bg-[#2DC8BE] py-2.5 text-sm font-semibold text-[#060D13] transition-all hover:bg-[#3DDDD3] disabled:opacity-50">
            {loading ? (
